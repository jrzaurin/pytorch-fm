...
def MyMovieLensSmallDataset(torch.utils.data.Dataset):
    """
    ...
    def __init__(self, dataset_path, sep=",", engine="c", header="infer"):
        ...
        # encode user and item id from 0 to max length
        data['userId'] = data['userId'].astype('category').cat.codes.values
        data['movieId'] = data['movieId'].astype('category').cat.codes.values

        data_arr = data.to_numpy()
        self.items = data_arr[:, :2].astype(np.int64)
        self.targets = self.__preprocess_target(data_arr[:, 2]).astype(np.float32)
